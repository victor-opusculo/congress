...
class SpectatorSubscriptionsClosureDate extends DataEntity
{
    public function __construct($initialValues = null)
    {
        $this->properties = (object)
        [
            'name' => new DataProperty('', 'SPECTATOR_SUBSCRIPTIONS_CLOSURE_DATE', DataProperty::MYSQL_STRING),
            'value' => new DataProperty('dateDate', null, DataProperty::MYSQL_STRING)
        ];

        $this->properties->name->setValue('SPECTATOR_SUBSCRIPTIONS_CLOSURE_DATE');
        $this->initializeWithValues($initialValues);
    }

    protected string $databaseTable = 'settings';
    protected string $formFieldPrefixName = 'settingsSpecSubsClosureDate';
