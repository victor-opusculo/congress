...
if (isset($_SESSION['assessor_id']) && $id)
{
    ...
    try
    {
        ...
        if ($article->idded_filename)
            throw new Exception('Não é possível desistir após a versão identificada ter sido anexa!');

        $article->evaluator_assessor_id = null;
        $article->status = ArticleStatus::WaitingAssessor->value;
        $article->is_approved = 0;
        
        $result = $article->save($conn);
        if ($result['affectedRows'] > 0)
            $messages[] = 'Você desistiu da avaliação do artigo!';
        else
            throw new Exception('Não foi possível desistr da avaliação.');
    }
    catch (Exception $e)
    {
